...
def get_one_stock_info(stock_list,row_count,sheet):
    ...
    for stock in stocks:
        ...
        if len(data)>1:
            process_per_stock(stock,row_count+i,sheet)
            row_count += 1
        else:
            useless_stock_num.append(str(data)[-9:-3])
    return useless_stock_num,row_count
...
for i in range(4): #4
    ...
    sh_useless,row_count = get_one_stock_info(sh_stock,row_count,ws1)
...
for i in range(6): #6
    ...
    sz_useless,row_count = get_one_stock_info(sz_stock,row_count,ws2)
...
new_filename = r'stock_basic2.xlsx'
